...
<script>
	$(document).ready(function(){
		...
		$("#list").click(function() {
			window.location.href="list.do?book_kind=all";
		});
		...
		$("#writeQna").click(function() {
			...
			var book_kind = $("#book_kind").val();
	 		...
	 		var query="qnaForm.do?book_id="+book_id;
	...
	function edit(editBtn) {
		...
		var arr = rStr.split(",");
		var query = "qnaUpdateForm.do?qna_id="+arr[0];
	    ...
	    $.ajax({
	       ...
	       success: function(data){
	    	   var str1 = '<p id="ck">';
	 		   ...
	 		   if (check == "1"){
	 		  	  alert("QnA가 삭제 되었습니다.");
	 		 	  var query = "bookContent.do?book_id="+arr[1];
...
<div id="cata" class="box2">
	<ul>
		<li><a href="list.do?book_kind=100">문학</a>
		<li><a href="list.do?book_kind=200">어학</a>
		<li><a href="list.do?book_kind=300">컴퓨터</a>
		<li><a href="list.do?book_kind=all">전체</a>
	</ul>
</div>
...
<div id="showBook">
	<table class="vhcenter"> 
		<tr height="30"> 
			<td rowspan="6"  width="150"> <img src="bookImage/${book.getBook_image()}" class="contentimage"></td> 
		...
		<tr>
			<td width="500">
				...
				<c:if test="${book.getBook_count()==0}"><p>일시품절</c:if>
				<c:if test="${book.getBook_count()>=1}">수량 : <input type="text" size="5" id="buy_count" value="1"> 개</c:if>
...
<div id="showQna">
	<p class="b">상품QnA 
		<c:if test="${!empty sessionScope.id}"><button id="writeQna">상품QnA쓰기</button> </c:if>
		<c:if test="${empty sessionScope.id}"><p>상품QnA를 쓰실려면 로그인 하세요.</p> </c:if>
	</p>
	<c:if test="${count == 0}">
		<ul>
			<li>등록된 상품QnA가 없습니다.
